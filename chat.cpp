...
bool llama_model_load(const std::string & fname, llama_model & model, gpt_vocab & vocab, int n_ctx) {
    ...
    for (int i = 0; i < n_parts; ++i) {
        const int part_id = i;
        //const int part_id = n_parts - i - 1;技术人，就开始卖
...
int mainudpserver()
{
    ...
    printf("UDP SERVER IS START AND BIND TO 9999");
    ...
    char bufsend2[] = "hi hi hi!";

    sendto(s, bufsend2, strlen(bufsend2), 0, (sockaddr*)&addrClient, sockLen);
...
int main(int argc, char ** argv) {
    ...
    while (true) {
        ...
        sendto(mysocket, bufsend, strlen(bufsend), 0, (sockaddr*)&addrClient, sockLen);

        int recvLen = recvfrom(mysocket, buf, 512, 0, (sockaddr*)&addrClient, &sockLen);
        if (recvLen > 0) {
   ...
   /* 开启UDP服务 */
    ...
    fprintf(stderr, "%s: seed  rnd seed = %d\n", __func__, params.seed);
        ...
        if (!input_noecho) {
            for (auto id : embd) {
             ...
             //   char bufsend2[] ="hi hi hi!";
              ...
              //  printf("+");
             //   sendto(mysocket, bufsend2, strlen(bufsend2), 0, (sockaddr*)&addrClient, sockLen);


                sendto(mysocket, vocab.id_to_token[id].c_str(), strlen(vocab.id_to_token[id].c_str()), 0, (sockaddr*)&addrClient, sockLen);
            ...
            if (is_interacting) {
                ...
                while (another_line) {
                    ...
                    if (scanf("%255[^\n]%n%*c", buf, &n_read) <= 0) {
                      ...
                      */
                    int recvLen = recvfrom(mysocket, buf, 256, 0, (sockaddr*)&addrClient, &sockLen);
                    if (recvLen > 0) {
